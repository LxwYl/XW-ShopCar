...
- (void)actionBtnDidClicked:(KDButton *)btn {
...
- (KDButton *)rowActionWithStyle:(LYSideslipCellActionStyle)style title:(NSString *)title {
    KDButton *actionBtn = [self dequeueReusableActionBtn];
...
- (KDButton *)dequeueReusableActionBtn {
    KDButton *actionBtn = nil;
    if (self.actionBtns.count > 0) {
        actionBtn = [self.actionBtns firstObject];
        [self.actionBtns removeObjectAtIndex:0];
    } else {
        actionBtn = [KDButton buttonWithType:UIButtonTypeCustom];
        actionBtn.contentEdgeInsets = UIEdgeInsetsMake(0, 20, 0, 10);
    }
    return actionBtn;
}
